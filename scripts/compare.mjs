...
async function captureScreenshot(url, outputPath, browser, viewport) {
  ...
  try {
    ...
    // Esconder o botão flutuante do WhatsApp para não falhar a comparação de VRT
    try {
      await page.addStyleTag({ content: '#wa-floating-btn, .whatsapp-wrapper { display: none !important; }' });
    } catch (e) {}

    // Estabilizar a página (forçar imagens eager, parar Swiper, pausar transições)
    try {
      await page.evaluate(async () => {
        // 1. Converter lazy loading para eager para carregar todas as imagens instantaneamente
        document.querySelectorAll('img[loading="lazy"]').forEach(img => {
          img.setAttribute('loading', 'eager');
        });

        // 2. Esperar todas as imagens carregarem completamente
        ...
        // 3. Parar autoplay do Swiper e resetar para o slide inicial
        document.querySelectorAll('.swiper-container, .swiper, .swiper-container-initialized').forEach(el => {
          if (el.swiper) {
            try {
        ...
        // 3b. Parar animações GSAP Marquee e resetar para o início
        document.querySelectorAll('.gsap-marquee-container').forEach(el => {
          if (el._marqueeTween) {
            try {
              el._marqueeTween.pause(0);
            } catch (err) {}
          }
        });

        // 4. Desativar transições e animações CSS
...
function compareImages(imgAPath, imgBPath, diffOutPath) {
  ...
  // Criar novos buffers de tamanho idêntico (cortados na menor dimensão) para evitar erro no pixelmatch
  const imgADataResized = Buffer.alloc(width * height * 4);
  const imgBDataResized = Buffer.alloc(width * height * 4);

  // Copiar linha por linha para garantir que as dimensões fiquem idênticas
  for (let y = 0; y < height; y++) {
    const srcOffsetA = y * imgA.width * 4;
    const srcOffsetB = y * imgB.width * 4;
    const destOffset = y * width * 4;
    imgA.data.copy(imgADataResized, destOffset, srcOffsetA, srcOffsetA + width * 4);
    imgB.data.copy(imgBDataResized, destOffset, srcOffsetB, srcOffsetB + width * 4);
  }

  const mismatch = pixelmatch(imgADataResized, imgBDataResized, diff.data, width, height, {
    threshold,
    includeAA: false,
    diffColor: [220, 38, 38],
    aaColor:   [251, 191, 36],
